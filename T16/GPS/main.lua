...
local function write_log(wgt)
	...
	if wgt.old_time_write + wgt.log_write_wait_time < now then
		...
		io.write(file,
		         wgt.coordinates_current .. "," .. time_power_on .. ", " .. wgt.gpsSATS .. ", " .. wgt.gpsALT .. ", " ..
										         wgt.gpsSpeed, "\r\n")
		io.close(file)
...
local function calc_Distance(LatPos, LonPos, LatHome, LonHome)
	...
	local a = math.pow(math.sin(d_lat / 2.0), 2) + math.cos(LatHome * d2r) * math.cos(LatPos * d2r) *
...
-- ############################################################################
local options = {{"TextColor", COLOR, BLACK}, {"LineColor", COLOR, WHITE}, {"Debug", BOOL, 0}}
...
local function background(wgt) return end
		...
		if (wgt.gpsLAT ~= wgt.gpsPrevLAT) and (wgt.gpsLON ~= wgt.gpsPrevLON) then
			...
			if (wgt.gpsLAT_H ~= 0) and (wgt.gpsLON_H ~= 0) then
				...
				if (wgt.gpsPrevLAT ~= 0) and (wgt.gpsPrevLON ~= 0) then
					...
					wgt.gpsTotalDist = rnd(tonumber(wgt.gpsTotalDist) +
													                       calc_Distance(wgt.gpsLAT, wgt.gpsLON, wgt.gpsPrevLAT, wgt.gpsPrevLON), 2)
...
function refresh(wgt, event, touchState)
	...
	elseif wgt.options.Debug == 1 then
		...
		if (type(wgt.gpsLatLon) == "table") then
			...
			lcl_drawText(wgt.zone.x + 5, wgt.zone.y + 45, "P lat: " .. rnd(wgt.gpsLatLon["pilot-lat"], 6), LEFT + SMLSIZE,
			             newtext_color)
			lcl_drawText(wgt.zone.x + 5, wgt.zone.y + 60, "P lon: " .. rnd(wgt.gpsLatLon["pilot-lon"], 6), LEFT + SMLSIZE,
			             newtext_color)
